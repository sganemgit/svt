...
class LmProtcolChangeStressFlow(LmStressFlow):
    ...
    def do_stress(self, dut, lp):
        self.log.info("performing {} stress".format(self.stress_type))
        self.log.info("performing protocol change stress on dut port {}".format(dut.port_number))
        self.log.info("performing protocol change stress on lp port {}".format(lp.port_number))
        ...
        for dut, lp in self.dut_lp_pairs:
            ...
            if self.phy_type in common_phy_types:
                for fec in dut.fec_dict[self.phy_type]:
                    for common_phy_type in common_phy_types:
                        if self.phy_type == common_phy_type:
                            continue
                        for common_fec in dut.fec_dict[common_phy_type]:
                            self.test_phy_type = common_phy_type
                            self.test_fec_type = common_fec
                            ...
                            if self.assert_link_status(dut, lp, self.phy_type, fec):
                                ...
                                if self.assert_link_status(dut, lp, common_phy_type, common_fec):
