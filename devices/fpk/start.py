...
if args.auto:
    if args.remote:
        devices = get_detected_devices("fpk", args.remote)
    else:
        devices = get_detected_devices("fpk")

    if devices:
        for device,info in devices.items():
            globals()[device] = fpk(info['device_number'],info['port_number'])
