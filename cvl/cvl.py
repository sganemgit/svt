...
class cvl:
    ...
    def GetGitVersion():
        '''This function return git version from file 'git_version.txt'
            argument: None
            return: git version (str)
        '''
        git_version_file = open('/home/laduser/performance/git_version.txt')
        line = git_version_file.readline().replace('\n','')
        git_version_file.close()
        git_version  = []

        for char in line:
            if (char.isdigit()) or (char == '.'):
                git_version.append(char)

        return ''.join(git_version)

    def Get_BER_Mac_Statistics():
        '''This function returns a dictinary with BER mac statistics 
            argument: None
            return:
                MacStatistics -- type dictionary containing: 
                    'TotalPacketRecieve'    
                    'TotalPacketTransmite'      
                    'Mac_link_speed'    
        '''
    ...
    def Mac_link_statistic_dicts(Mac_link_statistic_dict,Advance_mac_statistics):
        '''This function fill the dictinary from arguments with mac link statistics
            argument:
                Mac_link_statistic_dict 
                Advance_mac_statistics - flag
            return: None
    ...
    # def _AddDataToCSVLog(log_time,comment,test):
    #   '''This function add rows to csv Log 
    #       argument:
    #           log_time
    #           Comment
    #           Test -test name
    ...
    #       csv_data = []
    #       if test == "ttl":
    #           csv_data = [log_time, handler.custom_data['iter_num'], handler.custom_data['MacLinkStatus'], handler.custom_data['PhyLinkStatus'],handler.custom_data['PhyLinkSpeed'], comment]
    #           #csv_data = [log_time, handler.custom_data['iter_num'], handler.custom_data['GetMacLinkStatus'], handler.custom_data['GetInternalPcsLinkStatus'],handler.custom_data['GetPhyHostInterLinkStatus'], handler.custom_data['GetPhyLinkStatus'],handler.custom_data['AutonegComplete'], handler.custom_data['GetPhyLinkSpeed'], handler.custom_data['GetFVLPcsMode'], comment]
    #       elif test == "ber":
    #           csv_data = [log_time, handler.custom_data['iter_num'], handler.custom_data['MacLinkStatus'], handler.custom_data['PhyLinkStatus'], handler.custom_data['CRCERRS'], handler.custom_data['ILLERRC'], handler.custom_data['ERRBC'], handler.custom_data['MLFC'], handler.custom_data['MRFC'], handler.custom_data['RLEC'], handler.custom_data['RUC'], handler.custom_data['RFC'], handler.custom_data['ROC'], handler.custom_data['RJC'], handler.custom_data['MSPDC'],handler.custom_data['GetPRC64'],handler.custom_data['GetPRC127'],handler.custom_data['GetPRC255'],handler.custom_data['GetPRC511'],handler.custom_data['GetPRC1023'],handler.custom_data['GetPRC1522'],handler.custom_data['GetPRC9522'],handler.custom_data['GetPTC64'],handler.custom_data['GetPTC127'],handler.custom_data['GetPTC255'],handler.custom_data['GetPTC511'],handler.custom_data['GetPTC1023'],handler.custom_data['GetPTC1522'],handler.custom_data['GetPTC9522'], handler.custom_data['TX Throughput'], handler.custom_data['RX Throughput'],handler.custom_data['Fec_Corrected_Counter'],handler.custom_data['FEC_Uncorrected_Counter'],handler.custom_data['RS_FEC_corrected_codeword'],handler.custom_data['RS_FEC_uncorrected_codeword'],handler.custom_data['RS_FEC_corrected_symbol_lane0'],handler.custom_data['RS_FEC_corrected_symbol_lane1'],handler.custom_data['RS_FEC_corrected_symbol_lane2'],handler.custom_data['RS_FEC_corrected_symbol_lane3'],comment]
    ...
    def _AddDataToCSVLog(log_time,comment,test):
        '''This function for add rows to csv Log (internal log)
            argument:
                log_time - sample time of current row
                comment - for specific row
                test - "ber"/"ttl"
            return: none
        '''
        try:
            ...
            #csv_data = [log_time]
            if test == "ttl":
                for  i in range(len(Ttl_csv_titles_list)) :
                    if i==0 :
                        csv_data = [log_time]
                    elif i == (len(Ttl_csv_titles_list) -1) :
                        csv_data.append(comment)
                    else :  
                        #csv_data.append(handler.custom_data[Ttl_csv_titles_list[i]])
                        csv_data.append(handler.custom_data.get(Ttl_csv_titles_list[i],'N/A')) 
                    #s=str(i)+': Title:'+Ttl_csv_titles_list[i] 
                    #handler.serial_print(s)
                    i += 1
                
                #handler.serial_print(str(csv_data))#print csv_data
            elif test == "ber":
                for  i in range(len(Ber_csv_titles_list)) :
                    if i==0 :
                        csv_data = [log_time]
                    elif i == (len(Ber_csv_titles_list) -1) :
                        csv_data.append(comment)
                    else :  
                        csv_data.append(handler.custom_data.get(Ber_csv_titles_list[i],'N/A')) 
                    #s=str(i)+': Title:'+Ber_csv_titles_list[i] 
                    #handler.serial_print(s)
                    i += 1
    ...
    def _GetLogParams():
        '''This function Create debug log
            argument: None
            return:
                Log-- a list contains - GetMacLinkStatus,GetPhyLinkStatus,GetPhyLinkSpeed                       
        '''
        handler = get_handler()
        driver = self.driver
        GetMacLink_Status = GetMacLinkStatus()
        GetPhyLink_Status = GetPhyLinkStatus()
        
        handler.custom_data['MacLinkStatus'] = GetMacLink_Status
        handler.custom_data['PhyLinkStatus'] = GetPhyLink_Status
        link_speed = GetPhyLinkSpeed()
        handler.custom_data['PhyLinkSpeed'] = link_speed 
    ...
    def _LinkStableTest(link_stability_time,ttl_result):
        '''This function verify link stable 
            argument:
                link_stability_time - max time to stable link
                ttl_result
            return:
                link_stable- true/false 
        '''
        link_stable = True
        ...
        temp = _GetLogParams()
        _AddDataToCSVLog(ttl_result + (curr_time - start_time),'Stability Check',"ttl")

        TempParams = _GetLogParams()
        while ((curr_time - start_time) < link_stability_time):
            curr_time = time.time()

            getParams = _GetLogParams()
            if getParams != TempParams:
                _AddDataToCSVLog(ttl_result + (curr_time - start_time),'Changes in Stability Check',"ttl")
    ...
    def _Ttl(ttl_timeout, RestartAnSrc,AmIDut,iter_num):
        '''This function return TTL [sec]
            argument:
                ttl_timeout - max time for link up
                RestartAnSrc - "AQ"/"REG"
                AmIDut - True/False
        ...
        ############################################################################################
        temp = _GetLogParams()
        _AddDataToCSVLog(_time,'Before RestartAN',"ttl")
        ##########################################################################################

        #drop current link
        if AmIDut:
            handler.serial_print("Restart AN in DUT")
            RestartAn(RestartAnSrc)

        #start counter
        start_time = curr_time = time.time()
        while (GetMacLinkStatus('REG') == True ):
            #print 'waiting for link down'
            if ((curr_time - start_time) > ttl_timeout):
                handler.serial_print("link is down for 15s")
                link_down_flag = False
                break
            curr_time = time.time()
        if (link_down_flag):
            #start counter for real ttl after waiting to link down 
            start_time = curr_time = time.time()

        TempParams = _GetLogParams()
        _AddDataToCSVLog(_time,'After RestartAN',"ttl")

        #wait for link up
        link_status = "Link Down / TTL timeout Or Link is not Down"
        handler.custom_data['AQvsREG'] = False
        while ((curr_time - start_time) < ttl_timeout):
            curr_time = time.time()
        ################################################################################################    
            getParams = _GetLogParams()
            if getParams != TempParams:
                ...
                _AddDataToCSVLog(_time,'Waiting for Link up',"ttl")
            TempParams = getParams 
        ###################################################################################################
            
            if GetMacLinkStatus('REG'):
                if not GetMacLinkStatus('AQ'):
                    handler.serial_print('Warning: Link Indication mismatch. AQ is Down while REG is up')
                    handler.custom_data['AQvsREG'] = True
                curr_time = time.time()
                handler.serial_print('link up')
                link_status = "Link Up"
        ...
        ttl_time = curr_time - start_time
        temp = _GetLogParams()
        _AddDataToCSVLog(ttl_time,link_status,"ttl")

        #return time
        handler.serial_print("ttl: {} ".format(ttl_time))
        return ttl_time

    def MainTtl (test_param_dict):
        '''This function perform main TTL test 
            argument:
                test_param_dict -- type(dict): 
                    'ttl_timeout' -- (int) max time for link up 
                    'ttl_pass_criteria' -- (int) max time for pass the test
                    'link_stable_test_Flag' -- True/False
                    'link_stability_timeout' -- (int) max time to stable link
                    'RestartAnSrc' -- (string) "AQ"/"REG"
                    'iter_num' -- (int) iteration number
                    'AmIDut' -- True/False
                    'last_iter' -- True/False
                    passing parameters to test is done with the is dicionary
            return:
                test_res_dict -- type(dict):
                    'Test_Status' -- (string) "Pass"/"Fail"
                    'ttl_result' -- (int) ttl result
                    'Mac_link_statistic_dict' -- (dict) from GetMacStatistics(Advance) - advance will be True if the test status is Fail 
                    'Phy_link_statistic_dict' -- (dict) from GetPhyStatistics(Advance) - advance will be True if the test status is Fail
                    'phy_tuning_paraps_dict' -- (dict) from GetPhytuningParams()
                    'phy_info_dict' -- (dict) from DnlGetPhyInfo()
                    'Link_stable_status' -- (bool) True/False 
        '''

        ttl_timeout = test_param_dict["ttl_timeout"]
        ttl_pass_criteria = test_param_dict["ttl_pass_criteria"]
        link_stable_test_Flag = test_param_dict["link_stable_test_Flag"]
        link_stability_time = test_param_dict["link_stability_timeout"]
        RestartAnSrc = test_param_dict["RestartAnSrc"]
        iter_num = test_param_dict["iter_num"]
        AmIDut = test_param_dict["AmIDut"]
        last_iter = test_param_dict["last_iter"]
        ...
        LogName = "TTL_" + "CVL" + "_" + "Port" + "_" + str(port)
        handler = get_handler()
        ...
        if not 'csv_buffer' in handler.custom_data:
            ...
            #csv_titles = ["Time_Stamp[s]","iter_num","MacLinkStatus", "PhyLinkStatus","PhyLinkSpeed","Comments"] 
            csv_buffer = ut.CsvBuffer(statistics_file, Ttl_csv_titles_list)
        ...
        ttl_result = _Ttl(ttl_timeout, RestartAnSrc,AmIDut,iter_num)
        phy_tuning_paraps_dict = GetPhytuningParams()

        if (link_stable_test_Flag and handler.custom_data['LinkStatus']):
            Link_stable_status = _LinkStableTest(link_stability_time,ttl_result)
        else:
            Link_stable_status = 'N/A'

        handler.serial_print("Link_stable_status {}".format(Link_stable_status))
        
        # test pass 
        if (ttl_result < ttl_pass_criteria) and Link_stable_status and handler.custom_data['LinkStatus']:
            Test_Status = "Pass"    
            # read Mac link statistic
            Mac_link_statistic_dict = GetMacStatistics(False)
            #print Mac_link_statistic_dict
            Phy_link_statistic_dict = GetPhyStatistics(False)
            
        # test fail 
        else:
            Test_Status = "Fail"        
            # read Mac link statistic
            Mac_link_statistic_dict = GetMacStatistics(True)
            Phy_link_statistic_dict = GetPhyStatistics(True)
            
        phy_info_dict = DnlGetPhyInfo()
        Mac_link_statistic_dict['MAC_Link_Status_REQ_vs_AQ_Mismatch'] = handler.custom_data['AQvsREG']
        test_res_dict["Test_Status"] = Test_Status
        test_res_dict["ttl_result"] = ttl_result
        test_res_dict["Mac_link_statistic_dict"] = Mac_link_statistic_dict
        test_res_dict["Phy_link_statistic_dict"] = Phy_link_statistic_dict
        test_res_dict["phy_tuning_paraps_dict"] = phy_tuning_paraps_dict
        test_res_dict["phy_info_dict"] = phy_info_dict
        test_res_dict["Link_stable_status"] = Link_stable_status

        if last_iter:
            handler.serial_print(handler.custom_data['statistic_file'])
            handler.serial_print("sending file")
            handler.send_file(handler.custom_data['statistic_file'])    
        return  test_res_dict



    def _Globr(ttl_timeout,GlobRes):
        '''This function return TTL [sec] after globr
            argument:
        ...
        ############################################################################################
        _time = 0
       # temp = _GetLogParams("REG")
        #_AddDataToCSVLog(_time,'Before GlobalReset')
        if GlobRes == True:
        ##########################################################################################
            Reset(1)
        #start counter
        start_time = curr_time = time.time()
        while (GetMacLinkStatus("REG") == True and (curr_time - start_time) < ttl_timeout):
            print 'waiting for link down in port '
            curr_time = time.time()

        #TempParams = _GetLogParams("REG")
        #_AddDataToCSVLog(_time,'After GlobalReset')
        ...
        while ((curr_time - start_time) < ttl_timeout):
            curr_time = time.time()
        ################################################################################################    
            #getParams = _GetLogParams("REG")
            #if getParams != TempParams:
                #_time = curr_time - start_time
                #_AddDataToCSVLog(_time,'Waiting for Link up')
            #TempParams = getParams 
        ###################################################################################################
        ...
        ttl_time = curr_time - start_time
        #temp = _GetLogParams("REG")
        #_AddDataToCSVLog(ttl_time,link_status)

        #return time
        #print "ttl: " , ttl_time   
        return ttl_time

    def MainBER(test_param_dict):
        '''This function perform main BER test
            arguments: test_param_dict{BER_timeout,smaple_time,Packet_size,iter_num,last_iteration,specific_PTC,specific_PRC}
            argument:
                test_param_dict -- type(dict): 
                    'BER_timeout' -- (int) traffic time
                    'smaple_time' -- (int) sample time of the link
                    'Packet_size' -- (int) 
                    'iter_num' -- (int) iteration number
                    'last_iteration' -- (bool) True/False
                    'specific_PTC' -- (string) 'GetPTC64'/'GetPTC127'/'GetPTC255'/'GetPTC511'/'GetPTC1023'/'GetPTC1522'/'GetPTC9522'    
                    'specific_PRC' -- (string) 'GetPRC64'/'GetPRC127'/'GetPRC255'/'GetPRC511'/'GetPRC1023'/'GetPRC1522'/'GetPRC9522'
                    passing parameters to test is done with the is dicionary
            return:
                test_res_dict -- type(dict):
                    'Mac_Error_Dict' -- (dict)
                    'Total_FEC_Counter_dict' -- (dict)
                    'Mac_Statistics_Dict' -- (dict) from GetMacStats()
                    'TX_Throughput_Average' -- (int)
                    'RX_Throughput_Average' -- (int)
                    'phy_tuning_paraps_dict' -- (dict) from GetPhytuningParams()
                    'link_drop_flag' --(bool) True/False
                    'phy_info_dict' -- (dict) from DnlGetPhyInfo()
        '''
        
        ber_timeout = test_param_dict['BER_timeout']
        sample_time = test_param_dict['smaple_time']
        packet_size = test_param_dict['Packet_size']
        iter_num = test_param_dict['iter_num']
        last_iteration = test_param_dict['last_iteration']
        specific_PTC = test_param_dict['specific_PTC']
        specific_PRC = test_param_dict['specific_PRC']
        ...
        LogName = "BER_" + "CVL" + "_" + "Port" + "_" + str(port)
        handler = get_handler()
        handler.custom_data['iter_num'] = iter_num
        ...
        if not 'csv_buffer' in handler.custom_data:
            statistics_file = handler.create_file_name(LogName, 'csv', handler.get_device_key())
            handler.custom_data['statistic_file'] = statistics_file
            #csv_titles = ["Time_Stamp[s]","iter_num", "MacLinkStatus","PhyLinkStatus","CRCERRS","ILLERRC","ERRBC","MLFC","MRFC","RLEC","RUC","RFC","ROC","RJC","MSPDC",'GetPRC64','GetPRC127','GetPRC255','GetPRC511','GetPRC1023','GetPRC1522','GetPRC9522','GetPTC64','GetPTC127','GetPTC255','GetPTC511','GetPTC1023','GetPTC1522','GetPTC9522', "TX Throughput","RX Throughput","Fec_Corrected_Counter","FEC_Uncorrected_Counter","RS_FEC_corrected_codeword","RS_FEC_uncorrected_codeword","RS_FEC_corrected_symbol_lane0","RS_FEC_corrected_symbol_lane1","RS_FEC_corrected_symbol_lane2","RS_FEC_corrected_symbol_lane3","Comments"] 
            csv_buffer = ut.CsvBuffer(statistics_file, Ber_csv_titles_list)
            handler.custom_data['csv_buffer'] = csv_buffer # saving some value to custom data dictionary that can be retrieved in another function

        curr_time = 0
        start_txrx_aserted = False
        Mac_Error_Dict = {}
        Phy_Statistics_Dict = {}
        Mac_Statistics_Dict = {}
        TX_Throughput_List = []
        ...
        new_PTC_Dict = GetPTC()
        new_PRC_Dict = GetPRC()
        handler.serial_print("Iteration: {}".format(iter_num))
        handler.serial_print("PTC: {}".format(new_PTC_Dict['TotalPTC']))
        handler.serial_print("PRC: {}".format(new_PRC_Dict['TotalPRC']) )
        
        handler.custom_data['MacLinkStatus'] = CurrentMacLinkStatus_old_value
        handler.custom_data['PhyLinkStatus'] = CurrentPhyLinkStatus_old_value

        #FEC_Corrected_Counter, FEC_Uncorrected_Counter = GetFECCounter() 
        current_fec_stat = GetCurrentFECStatus() 
        FEC_counter_dict = GetFECCounter(current_fec_stat)
        for key,val in FEC_counter_dict.iteritems():
            #print key, "=>", val
            handler.custom_data[key] = val
            
        _FillCustomDataInMacErros(Mac_Error_Dict,True)
        _FillCustomDataInMacStatistics(0,0,new_PTC_Dict,new_PRC_Dict)
        _AddDataToCSVLog(0,"Before Traffic","ber")

        
        start_time = sampling_time_counter = time.time()
        while (curr_time < ber_timeout + 1):
            curr_time = time.time() - start_time
            #_FillCustomDataForBerTestByNa()
            if (time.time() - sampling_time_counter > sample_time):
                if not start_txrx_aserted:
                    handler.serial_print("Start TXRX")
                    EthStartTraffic(packet_size)
                    start_txrx_aserted = True
                    start_time = sampling_time_counter = time.time()
                
                sampling_time_counter = time.time()
                new_PTC_Dict = GetPTC()
                new_PRC_Dict = GetPRC()
                TX_Throughput = _GetThroughput(prev_PTC,int(new_PTC_Dict[specific_PTC]),sample_time,packet_size)
                TX_Throughput_List.append(TX_Throughput)
                RX_Throughput = _GetThroughput(prev_PRC,int(new_PRC_Dict[specific_PRC]),sample_time,packet_size)
                RX_Throughput_List.append(RX_Throughput)
                prev_PTC = new_PTC_Dict[specific_PTC]
                prev_PRC = new_PRC_Dict[specific_PRC]   
                ...
                for key,val in FEC_counter_dict.iteritems():
                    #print key, ":", val
                    if (val != 'N/A'):
                        if key in Total_FEC_Counter_dict.keys():
                            #handler.serial_print("key: {}".format(key))
                            #handler.serial_print("Total_FEC_Counter_dict[key]: {}".format(Total_FEC_Counter_dict[key]))
                            #handler.serial_print("val: {}".format(val))
                            int_val = int((Total_FEC_Counter_dict[key]))
                            int_val+=val
                            Total_FEC_Counter_dict[key] = int_val
                        else:
                            Total_FEC_Counter_dict[key] = val
                    else :
                        if key not in Total_FEC_Counter_dict.keys():
                            Total_FEC_Counter_dict[key] = val
                    handler.custom_data[key]=val
                
                #Total_Corrected_Counter += FEC_Corrected_Counter
                
                #Total_Uncorrected_Counter += FEC_Uncorrected_Counter
                #handler.custom_data['FEC_Corrected_Counter'] = Total_Corrected_Counter
                #handler.custom_data['FEC_Uncorrected_Counter'] = Total_Uncorrected_Counter
                
                _FillCustomDataInMacErros(Mac_Error_Dict,True)
                _FillCustomDataInMacStatistics(TX_Throughput,RX_Throughput,new_PTC_Dict,new_PRC_Dict)
                _AddDataToCSVLog((time.time() - start_time),"Traffic","ber")
            ...
            if (CurrentMacLinkStatus != CurrentMacLinkStatus_old_value or CurrentPhyLinkStatus != CurrentPhyLinkStatus_old_value):
                ...
                _AddDataToCSVLog((time.time() - start_time),"link is changed","ber")
        ...
        handler.serial_print("Stop TXRX")
        EthStopTraffic()

        handler.custom_data['MacLinkStatus'] = GetMacLinkStatus()
        handler.custom_data['PhyLinkStatus'] = GetPhyLinkStatus()
        new_PTC_Dict = GetPTC()
        new_PRC_Dict = GetPRC()
        ...
        _FillCustomDataInMacErros(Mac_Error_Dict,True)
        _FillCustomDataInMacStatistics(TX_Throughput,RX_Throughput,new_PTC_Dict,new_PRC_Dict)
        _AddDataToCSVLog(ber_timeout,"After Traffic","ber")

        handler.serial_print("PTC: {}".format(new_PTC_Dict['TotalPTC']))
        handler.serial_print("PRC: {}".format(new_PRC_Dict['TotalPRC'])) 
        Mac_Statistics_Dict = GetMacStats()
        ...
        #print Total_FEC_Counter_dict
        #preparing result for return
        test_res_dict['Mac_Error_Dict'] = Mac_Error_Dict
        test_res_dict['Total_FEC_Counter_dict'] = Total_FEC_Counter_dict
        test_res_dict['Mac_Statistics_Dict'] = Mac_Statistics_Dict
        test_res_dict['TX_Throughput_Average'] = TX_Throughput_Average
        test_res_dict['RX_Throughput_Average'] = RX_Throughput_Average
        test_res_dict['phy_tuning_paraps_dict'] = phy_tuning_paraps_dict
        test_res_dict['link_drop_flag'] = link_drop_flag
        test_res_dict['phy_info_dict'] = phy_info_dict
        return test_res_dict

    def MainGlobr (ttl_timeout,ttl_pass_criteria,link_stable_test_Flag,link_stability_time,GlobRes,iter_num,last_iter):
        '''This function perform main Globr test
        argument:
            ttl_timeout [Sec]
            link_stable_test_Flag - verify that the link is stable after TTL test
            ttl pass criteria
        ...
        return:
            list:
                1:test status, 2:TTL, 3:Mac_link_statistic_dict, 4:phy_info_dict, 5:Link_stable_status       
        '''
        ...
        LogName = "GLOBR_" + "CVL" + "_" + "Port" + "_" + str(port)
        handler = get_handler()
        handler.custom_data['iter_num'] = iter_num
        if not 'csv_buffer' in handler.custom_data:
            statistics_file = handler.create_file_name(LogName, 'csv', handler.get_device_key())
            handler.custom_data['statistic_file'] = statistics_file
            csv_titles = ["Time_Stamp[s]","iter_num","MacLinkStatus","Comments"] 
            csv_buffer = ut.CsvBuffer(statistics_file, csv_titles)
            handler.custom_data['csv_buffer'] = csv_buffer # saving some value to custom data dictionary that can be retrieved in another function

        ttl_result = _Globr(ttl_timeout,GlobRes)

        print "handler.custom_data['LinkStatus']: ",handler.custom_data['LinkStatus']
        if (link_stable_test_Flag and handler.custom_data['LinkStatus']):
            Link_stable_status = _LinkStableTest(link_stability_time,ttl_result)
            print "Link_stable_status ",Link_stable_status
            
        # test pass 
        if (ttl_result < ttl_pass_criteria):
            
            Test_Status = "Pass"    
            # read Mac link statistic
            Mac_link_statistic_dict = GetMacStatistics(False,"REG",1)
            #Phy_link_statistic_dict = GetPhyStatistics(False)
        ...
        # test fail 
            ...
            Test_Status = "Fail"        
            # read Mac link statistic
            Mac_link_statistic_dict = GetMacStatistics(True,"REG",1)
            #Phy_link_statistic_dict = GetPhyStatistics(True)
            
        phy_info_dict = DnlGetPhyInfo()

            # ,Phy_link_statistic_dict -- add to return dictionary --- return_ttl_test_results[3]- AQ not working with Global Reset
        if (link_stable_test_Flag and handler.custom_data['LinkStatus']):
            # return valus: 1:test status, 2:TTL, 3:Mac_link_statistic_dict, 4:Phy_link_statistic_dict ,5: phy_info_dict, 6:Link_stable_status
            return_ttl_test_results =  [Test_Status,ttl_result,Mac_link_statistic_dict,phy_info_dict,Link_stable_status]# TODO: delete this mark when will be phy reading indications
        else:
            # return valus: 1: test status, 2: TTL, 3: Mac_link_statistic_dict, 4:Phy_link_statistic_dict ,5: phy_info_dict
            return_ttl_test_results = [Test_Status,ttl_result,Mac_link_statistic_dict,phy_info_dict]# TODO: delete this mark when will be phy reading indications
